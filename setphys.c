...
int main(int argc, char *argv[]) {
  ...
  for (size_t i = 0; i < nr_pages; ++i) {
    ...
    log(LOG_INFO, "OLD [virt: 0x%lx] (pfn: 0x%lx) phys: 0x%lx", virt_addr,
        old_pfn, old_phys_addr);
    if (show_pte) {
    ...
    log(LOG_INFO, "NEW [virt: 0x%lx] (pfn: 0x%lx) phys: 0x%lx", virt_addr,
